...
RSpec.describe Interpreter do
  ...
  describe 'snowy_plain_initialize' do
    context 'when no option is given' do
      it 'initializes all variables with default values' do
        expect(interpreter.outer_circle_radius).to eq Interpreter::DEFAULT_OUTER_CIRCLE_RADIUS
        expect(interpreter.inner_circle_radius).to eq Interpreter::DEFAULT_INNER_CIRCLE_RADIUS
        expect(interpreter.hero_position_angle).to eq Interpreter::DEFAULT_ANGLE
        expect(interpreter.hero_distance_from_base).to eq Interpreter::DEFAULT_OUTER_CIRCLE_RADIUS
        expect(interpreter.hero_sight_angle).to eq Interpreter::DEFAULT_ANGLE
      end
    end
    ...
    context 'when options are given' do
      ...
      it 'initializes all variables with these options' do
        expect(interpreter.outer_circle_radius).to eq 1
        expect(interpreter.inner_circle_radius).to eq 2
        expect(interpreter.hero_position_angle).to eq 3
        expect(interpreter.hero_distance_from_base).to eq 4
        expect(interpreter.hero_sight_angle).to eq 5
      end
    end
  end
  ...
  describe 'snowy_plain_move_forward' do
    context 'when player has not reached the base yet' do
      ...
      context 'when player sight angle is equal zero' do
        before { expect(interpreter).to receive(:snowy_plain_move_to_direction).with(interpreter.hero_sight_angle) }
  ...
  describe 'snowy_plain_move_to_direction' do
    ...
    before do
      interpreter.hero_position_angle = 0
      interpreter.hero_distance_from_base = hero_distance_from_base
      interpreter.hero_sight_angle = hero_sight_angle
    end
    ...
    context 'sight angle is lower than 90 deg' do
      ...
      context 'half distance' do
        ...
        it 'changes hero angle and position' do
          expect(interpreter.hero_position_angle).to be > 0
          expect(interpreter.hero_distance_from_base).to be < hero_distance_from_base
          expect(interpreter.hero_sight_angle).to eq hero_sight_angle
        end
      end
      ...
      context 'on outer limit' do
        ...
        it 'changes hero angle and position' do
          expect(interpreter.hero_position_angle).to be > 0
          expect(interpreter.hero_distance_from_base).to eq hero_distance_from_base
          expect(interpreter.hero_sight_angle).to eq hero_sight_angle
        end
      end
    end
  ...
  describe 'snowy_plain_turn_left' do
    context 'when angle is more than zero' do
      before { interpreter.hero_sight_angle = 90 }
      before { interpreter.snowy_plain_turn_left }

      it 'decreases hero sight angle' do
        expect(interpreter.hero_sight_angle).to eq 89
      end
    end
    ...
    context 'when angle equals zero' do
      ...
      it 'sets hero sight angle to 359' do
        expect(interpreter.hero_sight_angle).to eq 359
      end
    end
  end
  ...
  describe 'snowy_plain_turn_right' do
    context 'when angle is less than 359' do
      before { interpreter.hero_sight_angle = 170 }
      before { interpreter.snowy_plain_turn_right }

      it 'increases hero sight angle' do
        expect(interpreter.hero_sight_angle).to eq 171
      end
    end

    context 'when angle equals 359' do
      before { interpreter.hero_sight_angle = 359 }
      before { interpreter.snowy_plain_turn_right }

      it 'sets hero sight angle to 0' do
        expect(interpreter.hero_sight_angle).to eq 0
      end
    end
  end
  ...
  describe 'snowy_plain_base_found?' do
    context 'when player is near base' do
      before { interpreter.hero_distance_from_base = Interpreter::MIN_DISTANCE_FROM_BASE }
      
      context 'when player looks at the base' do
        before { interpreter.snowy_plain_base_found? }

        before { expect(interpreter).to receive(:snowy_plain_victory) }
      end

      context 'when player does not look at the base' do
        before { interpreter.hero_sight_angle = Interpreter::MAX_SIGHT_ANGLE + 1 }

        before { interpreter.snowy_plain_base_found? }

        before { expect(interpreter).not_to receive(:snowy_plain_victory) }
      end
    end
